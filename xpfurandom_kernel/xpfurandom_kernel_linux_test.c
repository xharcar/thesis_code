...
static int __init xpfurandom_kernel_init(void){
	void* data;
	int c;
	pr_alert("XPFUrandom: init started, variables declared.\n");
	...
	if(c!=0){
		pr_alert("XPFUrandom: cleanup failed\n");
		return -3;
	}
	pr_alert("XPFUrandom: init successful.\n");
	return 0;
}

static void __exit xpfurandom_kernel_exit(void)
{
	pr_alert("XPFUrandom: Module unloaded.\n");
}
